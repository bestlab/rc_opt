...
a dcd file, or has been corrupted (e.g. by ascii ftp)."); 
	...
	if ( strncmp( typestr, "CORD", 4) == 0 ) {
		...
		if ( strncmp( typestr, "CORD", 4) == 0 ) {
			...
			type = 'c';
			sfbita = true;
			fprintf(stderr,"DCD file actually has 64 bit integers\n");
		} else if ( strncmp( typestr, "VELD", 4) == 0 ) {
 ...
 ************************************************************************/
DCDOTrajFile::DCDOTrajFile( DCDITrajFile & inptraj )
{
	setup( inptraj );
}
 ...
 ************************************************************************/
void DCDOTrajFile::setup( DCDITrajFile & inptraj)
{
...
void XTCITrajFile::read_frame(float *XX, float *YY, float *ZZ, int n)
{
	...
	//for (int i=0; i<6; i++) {
	//	fprintf(stdout,"crystal[%i] = %12.6e\n",i,crystal_data[i]);
	//}
	/*
	xdr_int(xdr,&tmpi);
	if (tmpi!=n) { 
		print_err( "XTCITrajFile::read_frame()", 
				"number of atoms in frame wrong");
#ifndef SWIG
		throw ReadErr();
#else
		exit(1);
	...
	*/
	//if ( xdr3dfcoord(xdr,XYZ,&N,&precision,'r') )
	int ii = xdr3dfcoord(xdr,XYZ,&N,&precision,'r');
	//if (!xdr3dfcoord(xdr,XYZ,&N,&precision,'r')) {
	//fprintf(stdout,"tmpi = %i\n", ii);
	if (!ii) {
		moredata = false;
	//	fprintf(stdout,"GOT HERE!!!\n");
	}
	//fprintf(stdout,"precision = %12.6e\n",precision);
	for (int i=0; i<N; i++) {
		XX[i] = XYZ[3*i]*10.;
		YY[i] = XYZ[3*i+1]*10.;
		ZZ[i] = XYZ[3*i+2]*10.;
		//fprintf(stdout,"%5i %8.3f %8.3f %8.3f\n",i,XX[i],YY[i],ZZ[i]);
	}
	/* attempt to read next frame header */
	//if ( ! first ) {
		xx = xdr_int(xdr,&m);
		//fprintf(stdout,"xx = %i",xx);
		xx = xdr_int(xdr,&m);
		xx = xdr_int(xdr,&m);
...
int XTCITrajFile::total_frames() // try not to use this function!
{ 
	int nframe;
	long spos = ftell(xtcfile);
	int saved_counter = counter;
	bool saved_moredata = moredata;
	float *X = new float[N];
	float *Y = new float[N];
	float *Z = new float[N];
	while (frames_left()) {
	...
	fseek(xtcfile,spos,SEEK_SET);
	moredata = saved_moredata;
...
BaseITrajFile* open_trajectory(const string &fs)
{
	//string fs(filename);
	int slen = fs.size();
	if (slen-fs.rfind(string(".dcd"))==4) {
		fprintf(stderr,"dcd file\n");
		return new DCDITrajFile(fs.c_str());
	} else if (slen-fs.rfind(string(".xtc"))==4) {
		fprintf(stderr,"xtc file\n");
		return new XTCITrajFile(fs.c_str());
	} else {
