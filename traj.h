...
class BaseITrajFile : public BaseFile
{
	public:
		...
		virtual int total_frames() = 0;	// total # of frames
...
class DCDITrajFile : public BaseITrajFile
{
	...
	public:
		...
		int total_frames() { return nframes; }
...
class XTCITrajFile : public BaseITrajFile
{
	...
	public:
		...
		void get_crystal_data( double [6] ) const;
		bool crystal() const { return false; }
		// other stuff
		/*
		int charmm_version() const { return version; }
		void goto_frame( int );
...
BaseITrajFile * open_trajectory(const string &filename);
